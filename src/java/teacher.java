...
class teacher extends User {
    ...
    private final int type = 2; // 用户属性
    private String username; // 用户名称
    private String password; // 用户密码
    private String Id; // 用户id
    private int level; // 用户等级
    private String reTime; //用户注册时间
    private double totalCost; // 用户总共花费
    ...
    // 找回密码功能
    void forget_password(teacher customer) {
    ...
    // 用户-展示购物车
    void displayProductCar(teacher customer) {
        if (customer.userlibrary != null) {
            System.out.println("列表详情: ");
            for (int i = 0; i < customer.userlibrary.size(); i++) {
                System.out.println(customer.userlibrary.get(i).toString());
            }
        } else
    ...
    // 用户-添加商品进购物车
    void addToCart(teacher customer) {
        // 显示商品列表供用户选择
        displayProducts();
        ...
        // 创建购物车
        if (customer.userlibrary == null) {
            customer.userlibrary = new ArrayList<>();
        }
        // 将商品添加到购物车
        selectedProduct.setAmount(quantity);
        customer.userlibrary.add(selectedProduct);
    ...
    // 用户-删除商品出购物车
    void removeFromCart(teacher customer) {
        // 显示购物车中的商品供用户选择
        displayProductCar(customer);
        ...
        if (confirm.equalsIgnoreCase("Y")) {
            ...
            if (customer.userlibrary.isEmpty()) {
                customer.userlibrary = null;
            }
    ...
    // 用户-修改购物车中的商品
    void updateCartProduct(teacher customer) {
        // 显示购物车中的商品供用户选择
        if (customer.userlibrary == null || customer.userlibrary.isEmpty()) {
    ...
    // 用户-付账
    void checkout(teacher customer) {
        if (customer.userlibrary == null || customer.userlibrary.isEmpty()) {
            System.out.println("购物车为空，无法进行付款操作。");
            return;
        }

        displayProductCar(customer);

            /*在实际应用中，可以调用支付接口来进行支付操作
            simulatePayment();
             */

        readHistory history = new readHistory(new ArrayList<>(customer.userlibrary));
        if (customer.readHistory == null) {
            customer.readHistory = new ArrayList<>();
        }
        customer.readHistory.add(history);
        writeHistory(history);

        updateProductQuantities(customer);

        // 清空购物车
        customer.userlibrary.clear();
        customer.userlibrary = null;
    ...
    // 更新购物车中商品的数量
    private void updateProductQuantities(teacher customer) {
        for (Book cartProduct : customer.userlibrary) {
            for (Book dbProduct : system.Products) {
    ...
    // 用户-查看历史
    void viewHistory(teacher customer) {
        if (customer.readHistory == null || customer.readHistory.isEmpty()) {
            System.out.println("没有购物历史记录。");
            return;
        }
        System.out.println("购物历史记录数量: " + customer.readHistory.size());
        int i = 1;
        for (readHistory history : customer.readHistory) {
    ...
    void writeHistory(readHistory history) {
        String shopHistory_path = "./version2.0/Database/excel/shopHistory.csv";
        // 更新文本中的商品信息
